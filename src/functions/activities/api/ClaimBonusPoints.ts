...
function resolveClaimAction(actions: Record<string, string>): string | undefined {
    if (actions.reportClaimAllPoints) return actions.reportClaimAllPoints
...
export class ClaimBonusPoints extends Workers {
    public async claimBonusPoints(maxAttempts = 5): Promise<ClaimBonusPointsResult> {
        const result: ClaimBonusPointsResult = {
            attempts: 0,
            acknowledged: 0,
            pointsGained: 0,
            exhausted: false
        ...
        if (!actionId) {
            this.bot.logger.warn(
                this.bot.isMobile,
                'CLAIM-BONUS-POINTS',
                'Skipping: no Ready-to-claim action id discovered in Rewards chunks'
            )
            return result
        }

        const attemptLimit = Math.max(1, Math.floor(maxAttempts))
        const dashboardUrl = URLs.rewards.dashboard
        const routeContext = {
            url: dashboardUrl,
        ...
        this.bot.logger.info(
            this.bot.isMobile,
            'CLAIM-BONUS-POINTS',
            `Starting ClaimBonusPoints | geo=${this.bot.userData.geoLocale} | currentBalance=${this.bot.userData.currentPoints}`
        )
        ...
        for (let attempt = 1; attempt <= attemptLimit; attempt++) {
            ...
            try {
                ...
                for (let check = 1; acknowledged && check < 3; check++) {
                    ...
                    const drawerDrained =
                        readyBefore != null && readyAfter != null && readyAfter < readyBefore
                    if (balanceChanged || drawerDrained || readyAfter === 0) break
                ...
                this.bot.logger.debug(
                    this.bot.isMobile,
                    'CLAIM-BONUS-POINTS',
                    `Claim pass ${attempt} | status=${status} | acknowledged=${acknowledged} | previousBalance=${oldBalance} | currentBalance=${newBalance} | pointsGained=${gainedPoints} | readyBefore=${readyBefore ?? 'unknown'} | readyAfter=${readyAfter ?? 'unknown'}`
                )
                ...
                this.bot.logger.info(
                    this.bot.isMobile,
                    'CLAIM-BONUS-POINTS',
                    `Claimed Ready to claim pass ${attempt} | pointsGained=${gainedPoints} | totalClaimed=${result.pointsGained} | currentBalance=${newBalance} | readyBefore=${readyBefore ?? 'unknown'} | readyAfter=${readyAfter ?? 'unknown'}`,
                    'green'
                )
        ...
        this.bot.logger.info(
            ...
            result.pointsGained > 0
                ...
                : `Nothing claimed | attempts=${result.attempts} | pointsGained=0 | currentBalance=${this.bot.userData.currentPoints}`,
