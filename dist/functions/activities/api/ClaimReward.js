...
class ClaimReward extends Workers_1.Workers {
    async claimReward(child, parentId) {
        const offerId = child.offerId;
        const actionId = this.bot.nextActions.reportActivity;
        if (!actionId) {
            this.bot.logger.warn(this.bot.isMobile, 'CLAIM-REWARD', `Skipping ${offerId}: "reportActivity" not discovered in bundle`);
            return;
        }
        if (!child.hash) {
        ...
        this.bot.logger.info(this.bot.isMobile, 'CLAIM-REWARD', `Claiming reward | offerId=${offerId} | geo=${this.bot.userData.geoLocale}`);
        try {
            ...
            this.bot.logger.debug(this.bot.isMobile, 'CLAIM-REWARD', `Claim response | offerId=${offerId} | status=${status} | acknowledged=${acknowledged} | pointsGained=${gained} | currentBalance=${newBalance}`);
            if (acknowledged) {
                if (gained > 0) {
                    this.bot.userData.currentPoints = newBalance;
                    this.bot.userData.gainedPoints = (this.bot.userData.gainedPoints ?? 0) + gained;
                }
                this.bot.logger.info(this.bot.isMobile, 'CLAIM-REWARD', `Reward claimed | offerId=${offerId} | status=${status} | pointsGained=${gained} | currentBalance=${newBalance}`, 'green');
            }
            else {
                this.bot.logger.warn(this.bot.isMobile, 'CLAIM-REWARD', `Claim not acknowledged by server | offerId=${offerId} | status=${status}`);
            }
            await this.bot.utils.wait(this.bot.utils.randomDelay(5000, 10000));
        }
        catch (error) {
            this.bot.logger.error(this.bot.isMobile, 'CLAIM-REWARD', `Error in claimReward | offerId=${offerId} | message=${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
